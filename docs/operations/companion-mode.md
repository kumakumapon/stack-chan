...
CoreS3 input uses PCM16 at 16 kHz, averaging native stereo channels into 20 ms mono frames. Capture is half duplex: it runs only while listening and stops during recognition, replies, other local audio, disconnect, and deactivation. The mic resumes after local TTS or buffered Gateway audio playback actually finishes. A reply arriving after stop cannot restart it.

Gateway PCM output is buffered per turn, capped at 256,000 base64 characters (about six seconds at the negotiated 16 kHz mono format). Prefer short replies or local TTS (`tts.type: none` on the Gateway). Incremental playback, full duplex, barge-in, and AEC are future work, as described in Issue #40's later phase.
