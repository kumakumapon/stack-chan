...
このブランチは **実機未検証**。stack-chan 側 MOD とブラウザー検証クライアントを実装する。MiniStack の Python backend、Node 常駐ブリッジ、MCP、音声認識、写真転送、タッチイベントは未実装。
...
5. 同じキーで接続し `STK-LP-...` を選ぶ。「笑顔」、首 0.05 rad / 1 秒、停止、切断の順で確認する。
...
- `head.set`: yawRad/pitchRad、durationMs 500〜3000。PoC 上限は yaw ±0.25 / pitch ±0.15 rad。機械的な全可動域を表す値ではない。
...
動作コマンドには priority 0〜3、ttlMs 100〜10000 が必要。TTL は受信から実行開始までの有効時間。最大待機8件、セッション内256 request IDまで保持する。重複IDは同じ応答、異なる内容でのID再使用は拒否。上限到達は `session-full`（再起動・再接続が必要）。本 PoC は長時間常駐用ではない。

PC クライアントは1秒ごとに state.get を送る。本体で3秒以上受信が途絶えると閉じる。切断・停止後は MOD を再起動して新規 session を開始する。自動再接続、接続断の即時検知、非同期 event ストリームは後続実装。
