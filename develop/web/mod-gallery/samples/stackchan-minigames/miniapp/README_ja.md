...
実行コードは既存の[`mini_app_sample`](https://github.com/kumakumapon/stack-chan/tree/develop/firmware/mods/examples/mini_app_sample)と[`stackchan_catch`](https://github.com/kumakumapon/stack-chan/tree/develop/firmware/mods/examples/stackchan_catch)を正本とし、`compose.mjs`でimport制限に適合する単一の`miniapp.ts`へ合成しています。各ゲームのルールや描画方式は、それぞれのREADMEを参照してください。
